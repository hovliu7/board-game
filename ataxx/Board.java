...
class Board {
    ...
    void clear() {
        _whoseMove = RED;
        int maxIndex = index((char) ('g' + 2), (char) ('7' + 2));
        for (int i = 0; i <= maxIndex; i++) {
            if (i == index('a', '1') || i == index('g', '7')) {
                _board[i] = BLUE;
    ...
    boolean legalMove(Move move) {
        ...
        if (move.row0() < '1' || move.row0() > '7'
                || move.row1() < '1' || move.row1() > '7') {
            return false;
        }
        int initialIndex = index(move.col0(), move.row0());
        if (_board[initialIndex] != _whoseMove) {
            return false;
        }
        if (_board[initialIndex] == EMPTY) {
            return false;
        }
        int finalIndex = index(move.col1(), move.row1());
        if (_board[finalIndex] != EMPTY) {
            return false;
        }
        if (_board[finalIndex] == BLOCKED) {
            return false;
        }
    ...
    boolean canMove(PieceColor who) {
        for (char col = 'a'; col <= 'g'; col++) {
            for (char row = '1'; row <= '7'; row++) {
                if (get(col, row) == who) {
                    for (char col1 = (char) (col - 2);
                         col1 <= col + 2; col1++) {
                        for (char row1 = (char) (row - 2);
                             row1 <= row + 2; row1++) {
                            if (get(col1, row1) == EMPTY) {
                                if (col1 >= 'a' && col1 <= 'g'
                                        && row1 >= '1' && row1 <= '7') {
                                    return true;
                                }
    ...
    void makeMove(Move move) {
        ...
        allMoves().add(move);
        int initIndex = index(move.col0(), move.row0());
        int finalIndex = index(move.col1(), move.row1());
        if (!move.isPass()) {
            if (move.isExtend()) {
                addUndo(finalIndex);
                _board[finalIndex] = _whoseMove;
                ...
                addUndo(initIndex);
                _board[finalIndex] = _whoseMove;
                _board[initIndex] = EMPTY;
                _numJumps++;
            }
            for (int col = move.col1() - 1; col <= move.col1() + 1; col++) {
                for (int row = move.row1() - 1; row <= move.row1() + 1; row++) {
                    if (get((char) col, (char) row) == opponent) {
                        int tempIndex = index((char) col, (char) row);
                        addUndo(tempIndex);
        ...
        if (_numJumps == JUMP_LIMIT
                || (!canMove(opponent) && !canMove(_whoseMove))
    ...
    void undo() {
        ...
        while (_undoSquares.peek() != null) {
            ...
            PieceColor currColor = get(index);
            if (currColor != EMPTY) {
                _numPieces[currColor.ordinal()]--;
            }
            if (oldColor != EMPTY) {
                _numPieces[oldColor.ordinal()]++;
            }
            _board[index] = oldColor;
        }
        _undoSquares.pop();
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        ...
        int linIndex = index(c, r);
        if (linIndex == index('a', '1')
                || linIndex == index('a', '7')
                || linIndex == index('g', '1')
                || linIndex == index('g', '7')) {
            return false;
